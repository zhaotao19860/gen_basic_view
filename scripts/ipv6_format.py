...
###############################################################################
# 功能：
#     1.将ipip.txtx格式化为统一格式，并输出到ipv6_format.temp；
#     2.将用户自定义文件格式化为统一格式，并输出到ipv6_format.temp；
# 输入：
#     1.ipip.txtx
#       格式：
#       ip地址段        国家   省   市  学校或单位  运营商  纬度       经度        所在时区代表城市  时区   中国行政区划代码 国际区号 国家代码 洲代码
#       2001:200::/32   中国  广东	*	*	       电信	   23.125178  113.280637  Asia/Shanghai   UTC+8  440000          86	      CN	  AP
#     2.user.xxxx(用户自定义文件)
#       IP地址段               所属搜索引擎/大洲/国家  所属省  所属运营商
#       409:8000::/32	       中国	                 北京	  移动
#       410:8000::-410:9000::  中国	                 北京	  移动
# 输出：
#     1.ipv6_format.temp
#       格式：
#       ip地址段        国家  省份  运营商
...
def printUsage():
    print("Usage: python ./ipv6_format.py\n")
    sys.exit(1)
        ...
        with codecs.open(ipip_file_name, 'r', 'UTF-8') as fr,\
            ...
            for line in fr:
                ...
                if not len(line) or line.startswith('#'):
                    continue
                tmp = line.split()
                # ip地址段 国家    省份    地级市   学校或单位 运营商
                # tmp[0]   tmp[1]  tmp[2]  tmp[3]  tmp[4]    tmp[5]
                # 保留地址
                if str_in_list(tmp[1], Reserved):
                    tmp[1] = '保留地址'
                    tmp[2] = tmp[5] = 'none'
                # 内部地址
                if str_in_list(tmp[1], Internal):
                    tmp[2] = tmp[5] = 'none'
                # 大洲地址
                if '亚太地区' in tmp[1]:
                    tmp[1] = '亚洲'
                ...
                # 处理支持的国家
                if str_in_list_exact(tmp[1], Countrys) and '中国' not in tmp[1]:
                    tmp[2] = tmp[5] = 'none'
                # 处理不支持的国家
                if str_not_in_list_exact(tmp[1], Countrys + Searchs + Continents + Reserved + Internal):
                    tmp[1] = '海外线路'
                    tmp[2] = tmp[5] = 'none'
                # 处理港澳台
                if '香港' in tmp[2] or '澳门' in tmp[2] or '台湾' in tmp[2]:
                    tmp[5] = 'none'
                # 处理中国不支持的运营商
                if '中国' in tmp[1] and \
                    '香港' not in tmp[2] and \
                        '澳门' not in tmp[2] and \
                        '台湾' not in tmp[2]:
                    if str_not_in_list(tmp[5], MainOperators + OtherOperators):
                        tmp[2] = 'none'
                        tmp[5] = '其他运营商'
                # 处理四大运营商的省份信息及运营商信息
                mainoper = str_in_list(tmp[5], MainOperators)
                if mainoper:
                    if '中国' in tmp[2]:
                        tmp[2] = '其他'
                    if mainoper == '广电' and str_not_in_list(tmp[2], BctvProvinces):
                        tmp[2] = '其他'
                    tmp[5] = mainoper
                # 将包含省份信息的铁通归入相应省份的移动
                if '铁通' in tmp[5]:
                    if '中国' not in tmp[2]:
                        tmp[5] = '移动'
                # 处理第三方运营商的省份信息及运营商信息
                otheroper = str_in_list(tmp[5], OtherOperators)
                if otheroper:
                    tmp[2] = 'none'
                    tmp[5] = otheroper
    ...
    try:
        with codecs.open(user_file_name, 'r', 'UTF-8') as fu,\
            ...
            for line in fu:
                ...
                # 将ip地址格式转换为cidr格式
                if '-' in tmp[0]:
                    start_end = tmp[0].split('-')
                    cidrs = netaddr.iprange_to_cidrs(
                        start_end[0], start_end[1])
                    for cidr in cidrs:
                        # 写入规范化文件
                        fw.writelines([str(cidr), '\t', tmp[1], '\t',
                                       tmp[2], '\t', tmp[3], '\n'])
                elif '/' in tmp[0]:
                    # 写入规范化文件
                    fw.writelines([tmp[0], '\t', tmp[1], '\t',
...
def main():
    ...
    log = script_path + '/../log/ipv6_format.log'
    mk_dir(log)
    logging.basicConfig(filename=log, level=logging.INFO, filemode='a',
                        format='%(asctime)s %(filename)s %(levelname)s %(message)s')
    logging.info('........run start........')
    ipip_file_name = script_path + "/../data/ipv6/ipip.txtx"
    mk_dir(ipip_file_name)
    format_file_name = script_path + "/../data/temp/ipv6_format.temp"
    mk_dir(format_file_name)
    data_dir = script_path + "/../data/ipv6/"
    user_file_names = []
