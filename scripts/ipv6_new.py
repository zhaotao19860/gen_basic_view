...
###############################################################################
# 功能：
#     1.根据ipv6_format.temp和auto.view.map生成new_ipv6_range.temp；
# 输入：
#     1.ipv6_format.temp
#       格式：
#       ip地址段        国家  省份  运营商
...
def printUsage():
    print("Usage: python ./ipv6_new.py\n")
    sys.exit(1)


def get_view_dict_fix(view_map_file_name):
    view_dict = dict()
    try:
        with codecs.open(view_map_file_name, 'r', 'UTF-8') as fv:
            for line in fv:
                ...
                if not len(line) or line.startswith('#'):
                    continue
                tmp = line.split()
                # 视图ID   视图名称  上级视图名称  所属国家/大洲  所属省   所属运营商
...
def gen_new_range(format_file_name, view_map_file_name, out_file_name):
    ...
    #view_dict = get_view_dict_fix(view_map_file_name)
    try:
...
def main():
    ...
    log = script_path + '/../log/ipv6_new.log'
    mk_dir(log)
